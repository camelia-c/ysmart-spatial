...
def __get_input_correlation__(tree,ret_dict):
    ...
    elif isinstance(tree,ystreespatial.GroupByNode) or isinstance(tree,ystreespatial.OrderByNode):
...
### step2: look for transit correlation
    ...
    for table_name in ret_dict.keys():
        new_node = ystreespatial.CompositeNode()
        new_node.pk_dict[table_name] = pk_dict[table_name]
        for node in ret_dict[table_name]:
            table_list = []
            if isinstance(node,ystreespatial.TwoJoinNode):
                filter_name = ""
                if isinstance(node.left_child,ystreespatial.TableNode):
                    tmp_name = ""
                ...
                if isinstance(node.right_child,ystreespatial.TableNode):
                    tmp_name = ""
...
### set compositeNode's map output list
            for x in table_list:
                ...
                for y in tmp_exp_list:
                    new_exp = ystreespatial.YRawColExp(tn,y.column_name)
                    new_exp.column_name = int(new_exp.column_name)
                    new_exp.column_type = y.column_type
                    if tn in new_node.mapoutput.keys():
                        if ystreespatial.list_contain_exp(new_node.mapoutput[tn],new_exp) is False:
...
### set mapfilter to reduce the data shuffled
                ...
                if tmp_where is not None:
                    col_list = []
                    ystreespatial.__get_func_para__(tmp_where.where_condition_exp,col_list)
                    for exp in col_list:
                    ...
                    consExp = ystreespatial.YConsExp(True,"BOOLEAN")
                    if tn in new_node.mapfilter.keys():
...
def gen_composite_child(new_node,child):
    ...
    if isinstance(child,ystreespatial.GroupByNode):
        if child.composite is not None:
            new_node.child_list.append(child.composite)
        elif not isinstance(child.child,ystreespatial.TableNode):
            new_node.child_list.append(child.child)

    elif isinstance(child,ystreespatial.TwoJoinNode):
        if child.left_composite is not None:
            new_node.child_list.append(child.left_composite)
        elif not isinstance(child.left_child,ystreespatial.TableNode):
        ...
        elif not isinstance(child.right_child,ystreespatial.TableNode):
...
def jfc_child_pk(new_node,child,pk_list):
    for i in range(0,len(new_node.child_list)):
        ...
        tmp_list = []
        if isinstance(child,ystreespatial.TwoJoinNode):
            if tmp_node == child.left_child or tmp_node == child.left_composite:
                for pk in child.get_pk()[0]:
                    for exp in child.left_child.select_list.tmp_exp_list:
                        tmp_exp = ystreespatial.__trace_to_leaf__(child.left_child,exp,False)
                ...
                for pk in child.get_pk()[1]:
                    for exp in child.right_child.select_list.tmp_exp_list:
                        tmp_exp = ystreespatial.__trace_to_leaf__(child.right_child,exp,False)
                        if tmp_exp.compare(pk) is True:
                            pk.column_name = child.right_child.select_list.tmp_exp_list.index(exp)
                            tmp_list.append(pk)
                            break
        else:
            if isinstance(tmp_node,ystreespatial.GroupByNode):
                for pk in pk_list[0]:
                    for exp in tmp_node.select_list.tmp_exp_list:
                        tmp_exp = ystreespatial.__trace_to_leaf__(tmp_node,exp,False)
                        if tmp_exp.compare(pk) is True:
                            pk.column_name = child.right_child.select_list.tmp_exp_list.index(exp)
                            tmp_list.append(pk)
                            break
...
def jfc_composite_select(new_node,child,pk_list):
    ...
    tn_alias = None
    if isinstance(child,ystreespatial.TwoJoinNode):
        if isinstance(child.left_child,ystreespatial.TableNode):
        ...
        if isinstance(child.right_child,ystreespatial.TableNode):
        ...
        if isinstance(child.child,ystreespatial.TableNode):
...
def job_flow_correlation(tree):
    ...
    if isinstance(tree,ystreespatial.TableNode):
        ...
        if jfc is True:
            if tree.composite is None:
                new_node = ystreespatial.CompositeNode()
                if isinstance(tree.child,ystreespatial.SelectProjectNode):
...
### insert the new node into the tree
        ...
        if left_jfc == True and right_jfc == True:
            ...
            if tree.left_composite is None:
                new_node = ystreespatial.CompositeNode()
                child = None
                if isinstance(tree.left_child,ystreespatial.SelectProjectNode):
...
### set the child list for the new node
                ...
                elif not isinstance(tree.right_child,ystreespatial.TableNode):
...
### set the select list for the new node
            ...
            if tree.right_composite is None:
                new_node = ystreespatial.CompositeNode()
                child = None
                if isinstance(tree.right_child,ystreespatial.SelectProjectNode):
...
### set the child list for the new node
                ...
                elif not isinstance(tree.left_child,ystreespatial.TableNode):
...
### set the select list for the new node
        ...
        if tree.dep == 0:
            ...
            for x in tree.child_list:
                if isinstance(x,ystreespatial.SelectProjectNode):
                    node = x.child
                else:
                    node = x
...
def remove_sp_node(tree):
    ...
    if isinstance(tree,ystreespatial.TableNode):
        ...
        if tree.parent is None:
            return
        if isinstance(tree.parent,ystreespatial.TwoJoinNode):
