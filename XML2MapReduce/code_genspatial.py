...
spatial_func_list=["ST_AREA","ST_PERIMETER","ST_LENGTH","ST_CENTROID","ST_BOUNDARY","ST_INTERSECTION","ST_UNION","ST_DIFFERENCE","ST_DISTANCE","ST_CONTAINS","ST_CROSSES","ST_DISJOINT","ST_EQUALS","ST_INTERSECTS","ST_OVERLAPS","ST_TOUCHES","ST_WITHIN","ST_ASTEXT"]
...
def util_write_spatial_code(para1,buf_dict):
    return_str1=''
    util_wkt=''
    res_str=''
    
    if isinstance(para1,ystreespatial.YRawColExp):
        return_str1= __para_to_java__(para1.column_type,para1.column_name,buf_dict[para1.table_name])
        
        util_wkt='(new String('+return_str1+')).substring((new String('+return_str1+')).indexOf(\"\'\")+1,(new String('+return_str1+')).lastIndexOf(\"\'\"))'
    elif isinstance(para1,ystreespatial.YFuncExp):
        
        return_str1= __select_func_convert_to_java__(para1, buf_dict)
        
        util_wkt='(new String('+return_str1+')).substring'
        util_wkt+='((new String('+return_str1+')).indexOf(\"\'\")+1,(new String('+return_str1+')).lastIndexOf(\"\'\"))'
    else:
        #a constant
        if isinstance(para1,ystreespatial.YSpatialConsExp):
            return_str1= para1.cons_value["content"]
        else:
            return_str1= para1.cons_value
        
        pos1=return_str1.find('\'')
       
        pos2=return_str1.find('\'',pos1+1)
        
        util_wkt='"'+return_str1[pos1+1:pos2]+'"'
...
def write_spatial_code(exp,buf_dict):
    ...
    if len(exp.parameter_list) == 1:
        ...
        if exp.func_name=='ST_AREA':
            res_str+='.getArea()'
        elif exp.func_name=='ST_PERIMETER':
            res_str+='.getBoundary().getLength()'
        elif exp.func_name=='ST_LENGTH':
            res_str+='.getLength()'
...
def __select_func_convert_to_java__(exp,buf_dict):
    ...
    if not isinstance(exp,ystreespatial.YFuncExp):
        return
...
##### +, -, *, / operations
        ...
        if isinstance(para,ystreespatial.YRawColExp):
            return_str += __para_to_java__(para.column_type,para.column_name,buf_dict[para.table_name])

        elif isinstance(para,ystreespatial.YFuncExp):
            return_str += "("
        ...
        return_str += write_spatial_code(exp,buf_dict)
...
def __where_convert_to_java__(exp,buf_dict):
    ...
    if isinstance(exp,ystreespatial.YFuncExp):
        ...
        if exp.func_name in rel_func_dict.keys() or exp.func_name in math_func_dict.keys():
            ...
            for tmp_exp in exp.parameter_list:
                ...
                if isinstance(tmp_exp,ystreespatial.YRawColExp):
            ...
            if exp.func_name == "IS":
                ...
                if isinstance(para2,ystreespatial.YConsExp):
    ...
    elif isinstance(exp, ystreespatial.YConsExp):
...
sql_type_to_java = {"INTEGER":"IntWritable","DECIMAL":"DoubleWritable","TEXT":"Text","DATE":"Text",'ST_GEOMETRY':'Text'}
...
def __get_key_value_type__(exp_list):
    ...
    if len(exp_list) !=1:
        ...
        if isinstance(exp,ystreespatial.YRawColExp):
            
            res = sql_type_to_java[exp.column_type]

        elif isinstance(exp,ystreespatial.YFuncExp):
...
def __gen_des__(fo):
    print >>fo,"/*\n\
        ...
        Author for spatial extension: Camelia Ciolac   \n\
...
def __get_max_index__(exp_list):
    ...
    for exp in exp_list:
        if isinstance(exp,ystreespatial.YRawColExp):
            ...
            col_list = []
            ystreespatial.__get_func_para__(exp,col_list)
            for x in col_list:
...
def __orderby_gen_mr__(tree,fo):
    ...
    if isinstance(tree.child,ystreespatial.TableNode):
    ...
    if not isinstance(tree.child,ystreespatial.TableNode) or tree.child.where_condition is None:
            tmp_output = "\t\t\tcontext.write("
            tmp_output += "new " + map_key_type + "(" + map_key +")"
            tmp_output += ","
            tmp_output += "new " + map_value_type + "(" + map_value + ")"
...
##### groupby map part 
    ...
    for exp in gb_exp_list:
        if ystreespatial.__groupby_func_name__(exp) == "COUNT_DISTINCT":
    ...
    if config.advanced_agg is True:
        ...
        for exp in gb_exp_list:
            if ystreespatial.__groupby_func_name__(exp) == "AVG":
                map_value_type = "Text"
    ...
    if config.advanced_agg is True:
        ...
        for i in range(0,len(gb_exp_list)):
            exp = gb_exp_list[i]
            func_name = ystreespatial.__groupby_func_name__(exp)
            if func_name == "AVG":
...
### map part agg
        ...
        if isinstance(tree.child,ystreespatial.TableNode) and tree.child.where_condition is not None:
            where_str = "\t\t\tif("
            where_str +=__where_convert_to_java__(tree.child.where_condition.where_condition_exp,buf_dict)
            where_str += "){\n"
            print >>fo,where_str
        ...
        for i in range(0,len(gb_exp_list)):
            ...
            if isinstance(tree.child,ystreespatial.TableNode):
                ...
                col_list = []
                ystreespatial.__get_func_para__(tmp_exp,col_list)
                for x in col_list:
...
### no map agg
        ...
        for i in range(0,len(gb_exp_list)):
            ...
            tmp_name = ystreespatial.__groupby_func_name__(exp)
...
## map part agg
    ...
    if tree.having_clause is not None:
        ystreespatial.__get_gb_list__(tree.having_clause.where_condition_exp,col_list)
    ...
    for i in range(0,len(tree.select_list.tmp_exp_list)-having_len):
        ...
        elif isinstance(exp,ystreespatial.YRawColExp): 
        ...
        for tmp in tmp_list:
            for exp in gb_exp_list:
                if tmp.compare(exp) is True:
                    ...
                    new_exp = ystreespatial.YRawColExp("AGG",exp_index)
...
def __get_join_key__(exp,col_list,table):
    ...
    if exp is None or not isinstance(exp,ystreespatial.YFuncExp):
        return
    ...
    if len(exp.parameter_list ) ==2:
        ...
        tmp_bool = True
        if not isinstance(para1,ystreespatial.YRawColExp):
            tmp_bool = False
        if not isinstance(para2,ystreespatial.YRawColExp):
            tmp_bool = False
    ...
    for x in exp.parameter_list:
        if isinstance(x,ystreespatial.YFuncExp):
...
def __gen_func_exp__(exp,table_name):
    ...
    for x in exp.parameter_list:
        if isinstance(x,ystreespatial.YRawColExp):
            if x.table_name != table_name:
                tmp_exp = ystreespatial.YConsExp("\"NULL\"","TEXT")
                new_list.append(tmp_exp)
            else:
                new_list.append(x)
        elif isinstance(x,ystreespatial.YFuncExp):
    ...
    ret_exp = ystreespatial.YFuncExp(exp.func_name,new_list)
...
def __gen_join_where__(cur_exp,table_name):
    ...
    if not isinstance(cur_exp,ystreespatial.YFuncExp):
        return None

    if cur_exp.func_name in bool_func_dict.keys():
        for x in cur_exp.parameter_list:
            if not isinstance(x,ystreespatial.YFuncExp):
            ...
            if ret_exp == None:
                ...
                ret_exp = ystreespatial.YFuncExp(cur_exp.func_name,para_list)
...
###fix me here: how to handle the first para if the func is IS 
        if cur_exp.func_name == "IS":
            ...
            if tmp_bool == True:
                ...
                ret_exp = ystreespatial.YConsExp("FALSE","BOOLEAN")
...
### get map output key
    ...
    if len(left_key_list)  == 0:
        new_exp = ystreespatial.YConsExp(1,"INTEGER")
    ...
    if self_join_bool is False:
        ...
        if isinstance(tree.left_child,ystreespatial.TableNode):
        ...
        if isinstance(tree.right_child,ystreespatial.TableNode):
...
### scan the input of the left child
    ...
    if tree.left_child.select_list is not None:
        ...
        if not isinstance(tree.left_child,ystreespatial.TableNode) or tree.left_child.where_condition is None:
...
### scan the input of the right child
    ...
    if tree.right_child.select_list is not None:
        ...
        if not isinstance(tree.right_child,ystreespatial.TableNode) or tree.right_child.where_condition is  None:
...
def __composite_gen_mr__(tree,fo):
    ...
    for node in tree.child_list:
        ...
        for exp in tree.pk_dict[tn][0]:
            exp.table_name = tn
        exp_list = None
        if isinstance(node,ystreespatial.CompositeNode):
        ...
        for tmp in exp_list:
            ...
            new_exp = ystreespatial.YRawColExp(tn,tmp_in)
        ...
        for x in tree.mapfilter.keys():
            ...
            for y in tree.mapfilter[x]:
                if where_exp is None:
                    ...
                    para_list.append(y)
                    where_exp = ystreespatial.YFuncExp("OR",para_list)
...
#### dispatch #####
            ...
            for x in tree.it_node_list:
                if isinstance(x,ystreespatial.GroupByNode):
                ...
                elif isinstance(x,ystreespatial.TwoJoinNode):
                    self_join_bool = __self_join__(x)
                    if isinstance(x.left_child,ystreespatial.TableNode):
                    ...
                    if isinstance(x.right_child,ystreespatial.TableNode):
    ...
    for x in tree.jfc_node_list:
        if isinstance(x,ystreespatial.GroupByNode):
            ...
            for i in range(0,len(x.select_list.tmp_exp_list)-having_len):
                ...
                if ystreespatial.__groupby_func_name__(exp) not in agg_func_list:
                    if isinstance(exp,ystreespatial.YRawColExp):
            ...
            elif isinstance(x.left_child,ystreespatial.TableNode):
            ...
            elif isinstance(x.right_child,ystreespatial.TableNode):
...
def generate_code(tree,filename):
    ...
    if isinstance(tree,ystreespatial.TableNode):
        ...
        fo = open(op_name,"w")
        print 'generated code for TableNode in file:'+op_name
        __tablenode_code_gen__(tree,fo)
         
        
        return ret_name

    elif isinstance(tree,ystreespatial.OrderByNode):
        
        tree.output = filename
        fo = open(op_name,"w")
        print 'generated code for OrderByNode in file:'+op_name
        
        __orderby_code_gen__(tree,fo)
        if tree.composite is None:
            if not isinstance(tree.child,ystreespatial.TableNode):
    ...
    elif isinstance(tree,ystreespatial.SelectProjectNode):
        tree.output = filename
        fo = open(op_name,"w")
        print 'generated code for SelectProjectNode in file:'+op_name
        
        ret_name = generate_code(tree.child,filename)
        return ret_name

    elif isinstance(tree,ystreespatial.GroupByNode):
        
        tree.output = filename
        fo = open(op_name,"w")
        print 'generated code for GroupByNode in file:'+op_name
...
### to make sure that the CompositeNode is only visited once
        ...
        tree.output = filename
        fo = open(op_name,"w")
        print 'generated code for CompositeNode in file:'+op_name
        ...
        if len(tree.child_list) > 0:
            ...
            for x in tree.child_list:
                if isinstance(x,ystreespatial.TableNode):
                    continue
...
def ysmart_code_gen(argv,input_path,output_path):
    ...
    tree_node = ystreespatial.ysmart_tree_gen(argv[1],argv[2])
    print ''
    print ''
    print '....................................................................'
    ...
    if config.turn_on_correlation is True:
        ...
        tree_node = correlationspatial.ysmart_correlation(tree_node)
