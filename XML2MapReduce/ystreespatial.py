...
class YExpTool:
    ...
    token_list_spatial_funct= ["'ST_AREA'","'ST_PERIMETER'","'ST_LENGTH'","'ST_CENTROID'","'ST_BOUNDARY'","'ST_INTERSECTION'","'ST_UNION'","'ST_DIFFERENCE'","'ST_DISTANCE'","'ST_CONTAINS'","'ST_CROSSES'","'ST_DISJOINT'","'ST_EQUALS'","'ST_INTERSECTS'","'ST_OVERLAPS'","'ST_TOUCHES'","'ST_WITHIN'","'ST_ASTEXT'"]
...
# the get_pk function should be called after generating the index
    ...
    def debug(self, level):
        ...
        if oc is None:
            if swc is None:
                if sscs is None:
                    print pb, "TwoJoinNode", "[None]", "[None]", "[None]"
                else:
                    print pb, "TwoJoinNode", "[" + sscs + "]", "[None]", "[None]"
            else:
                if sscs is None:
                    print pb, "TwoJoinNode", "[None]", "[" + swc + "]", "[None]"
                else:
                    print pb, "TwoJoinNode", "[" + sscs + "]", "[" + swc + "]", "[None]"  
        else:
            if swc is None:
                if sscs is None:
                    print pb, "TwoJoinNode", "[None]", "[None]", "[" + oc + "]"
                else:
                    print pb, "TwoJoinNode", "[" + sscs + "]", "[None]", "[" + oc + "]"
            else:
                if sscs is None:
                    print pb, "TwoJoinNode", "[None]", "[" + swc + "]", "[" + oc + "]"
                else:
...
def process_schema_in_a_file(schema_name):
    ...
    for al in als:
        ...
        for a_tmp_col_raw in t_al_a[1:]:
            ...
            if a_column_type not in ["INTEGER", "DECIMAL", "TEXT", "DATE","ST_GEOMETRY"]:
...
def debug_global_tables():
    ...
    print '______________________GLOBAL DATA DICTIONARY______________________'
...
def __schema_select_list__(select_list,table_list,table_alias_dict):
    ...
    select_spatial_func_list=["ST_AREA","ST_PERIMETER","ST_LENGTH","ST_CENTROID","ST_BOUNDARY","ST_INTERSECTION","ST_UNION","ST_DIFFERENCE","ST_DISTANCE","ST_CONTAINS","ST_CROSSES","ST_DISJOINT","ST_EQUALS","ST_INTERSECTS","ST_OVERLAPS","ST_TOUCHES","ST_WITHIN","ST_ASTEXT"]
    if select_list is None:
        return res
    ...
    for x in select_list.dict_exp_and_alias.keys():
        if isinstance(x,YRawColExp):
            ...
            if (x.func_name.upper() not in select_func_list) and (x.func_name.upper() not in select_spatial_func_list):
                return -1
