...
// Extended/ Adapted for spatial features by Camelia C camelie_1985@yahoo.com //
// Time: 05-20-2012                                                      //
/*************************************************************************/
...
#include "YSmartSpatialLexer.h"
#include "YSmartSpatialParser.h"


extern pANTLR3_UINT8  YSmartSpatialParserTokenNames[];
...
void TreeWalkWorkerXML(pANTLR3_BASE_TREE p, int Level)
{
	...
	for  (c = 0; c<n ; c++) {
		...
		printf("<node tokentype=\"%d\" tokenname=\"%s\" line=\"%d\" positioninline=\"%d\" childcount=\"%d\">\n",
		       TokenType,
		       YSmartSpatialParserTokenNames[TokenType],
		       child->getLine(child),
...
int ANTLR3_CDECL main(int argc, char *argv[])
{
	...
	pYSmartSpatialLexer lxr;

	pANTLR3_COMMON_TOKEN_STREAM tstream;

	pYSmartSpatialParser psr;

	YSmartSpatialParser_start_rule_return langAST;
	...
	lxr = YSmartSpatialLexerNew(input);	    
	...
	psr = YSmartSpatialParserNew(tstream);
